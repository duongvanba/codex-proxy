...
> Tested trên macOS với Codex `26.609.30741` và `26.609.41114` (Electron 42.1.0 / Chromium framework 149). Tên file JS thay đổi theo version — dùng `ls` để xác định đúng tên.

### Bảng ánh xạ tên hàm/biến theo version

| Vai trò | `v0.133.0-alpha.1` | `26.609.30741` | `26.609.41114` |
|---|---|---|---|
| Mobile gate (app-main) | `Qy` / `Yy` | `BT` | `BT` |
| RemoteConnections selector (app-main) | `Xv=Wr(q,…)` | `Nw=c(g,({get:e})=>…)` | `Nw=c(g,({get:e})=>…)` |
| Visibility gates (remote-connection-visibility) | `d()`, `f()` | `c()` (export `n`), `l()` (export `r`) | `c()`, `l()` (giống) |
| Device-key factory (main) | `SU` / `OV` | `xZ` | **`wZ`** |
| Native loader / filename const / payload builder | `vU` / `yU` / `CU` | `_Z` / `vZ` / `SZ` | **`bZ` / `xZ` / `TZ`** |
| Module vars os/path/fs/crypto (main) | `r` / `i` / `o` / `s` | `i` / `a` / `s` / `c` | `i` / `a` / `s` / `c` (giống) |

> ⚠️ Tên trên CHỈ đúng cho từng version. Mỗi version phải grep lại (xem từng bước). Đặc biệt **device-key factory + native loader + filename const + payload builder XOAY tên mỗi build** (30741: `xZ`/`_Z`/`vZ`/`SZ` → 41114: `wZ`/`bZ`/`xZ`/`TZ` — lưu ý `xZ` đổi từ "factory" sang "filename const"!). Module var os/path/fs/crypto may ổn định `i/a/s/c` từ 30741→41114 nhưng vẫn phải grep — sai biến là crash main process.

### File JS ở `26.609.41114` (tham khảo — hash đổi mỗi build)

| File | 41114 |
|---|---|
| main process | `.vite/build/main-DFegGFWC.js` |
| app-main | `webview/assets/app-main-fcIxOLz5.js` |
| remote-connection-visibility | `webview/assets/remote-connection-visibility-Bqgndcez.js` |
| settings-page | `webview/assets/settings-page-0zwLIIUb.js` |
| remote-connections-settings | `webview/assets/remote-connections-settings-CBARKfIv.js` |

> ⚠️ **`settings-page` có 2 switch chứa `case\`plugins-settings\`:`** — switch #1 là visibility (`case\`…\`:return!1;`, ép `!1`→`!0`), switch #2 là fall-through `case…case…:I=!1` — **KHÔNG chèn `return!0;` vào switch #2** (phá fall-through → crash). Ở 41114 `keyboard-shortcuts` đã `return!0` sẵn trong switch #1.
...
# 1. Repack ASAR đã patch — ⚠️ PHẢI GIỮ unpacked native modules
#    app.asar gốc mark ~820 file là "unpacked" (better-sqlite3, node-pty, objc-js,
#    @worklouder) — load native .node TỪ ĐĨA. `asar pack` TRẦN sẽ nhúng chúng vào
#    asar → require .node fail. PHẢI truyền --unpack-dir đúng các package đó.
#    Grep danh sách unpacked thật từ header gốc trước khi pack (xem ô bên dưới).
~/.bun/bin/bunx @electron/asar pack codex-ext-2609 /tmp/app-patched.asar \
  --unpack-dir "node_modules/{@worklouder,better-sqlite3,node-pty,objc-js}"
# Lệnh trên tạo CẢ /tmp/app-patched.asar.unpacked/ — phải copy CẢ HAI vào bundle.
NEW_HASH=$(shasum -a 256 /tmp/app-patched.asar | awk '{print $1}'); echo "hash=$NEW_HASH"

# Verify unpack set khớp gốc (0 missing là bắt buộc; extra trong @worklouder vô hại):
~/.bun/bin/bun -e '
const {readFileSync}=require("fs");
const u=p=>{const fd=readFileSync(p),hs=fd.readUInt32LE(12);let j=fd.subarray(16,16+hs).toString("utf8");j=j.slice(0,j.lastIndexOf("}")+1);const r=JSON.parse(j),s=new Set();(function w(n,pre){if(!n.files)return;for(const[k,v]of Object.entries(n.files)){const p=pre+"/"+k;if(v.files)w(v,p);else if(v.unpacked)s.add(p);}})(r,"");return s;};
const a=u(process.env.HOME+"/Downloads/Codex-patched.app/Contents/Resources/app.asar"),b=u("/tmp/app-patched.asar");
const miss=[...a].filter(x=>!b.has(x));console.log("orig",a.size,"new",b.size,"missing",miss.length);if(miss.length){console.log(miss.slice(0,5));process.exit(1);}console.log("✅ 0 missing");'
...
# 3. Thay asar + app.asar.unpacked (CẢ HAI) + update Info.plist hash
#    (DÙNG /usr/bin/python3, KHÔNG homebrew python)
cp /tmp/app-patched.asar Resources/app.asar
rm -rf Resources/app.asar.unpacked
cp -R /tmp/app-patched.asar.unpacked Resources/app.asar.unpacked
...
| App mở được nhưng lỗi load native (`better-sqlite3`/`node-pty` .node), DB không lên, terminal pty chết | `asar pack` TRẦN nhúng native modules vào asar (gốc mark ~820 file unpacked) | Repack với `--unpack-dir "node_modules/{@worklouder,better-sqlite3,node-pty,objc-js}"` + copy CẢ `app.asar.unpacked` vào bundle (Bước 4.1/4.3) |
