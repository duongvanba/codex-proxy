...
export class AccountService {
  ...
  constructor(
    ...
    // Warmup request khi thêm account mới.
    this.accounts.onNewAccount((account) => {
      console.log(`[accounts] Warmup new account: ${account.email}`);
      void this.warmupAccount(account).finally(() => void this.fetchQuota());
    });
  }
  ...
  private async warmupAccount(account: Account): Promise<void> {
    ...
    try {
      ...
      await res.text();
      console.log(`[accounts] Warmup ${account.email}: HTTP ${res.status}`);
    } catch (e) {
      console.warn(`[accounts] Warmup ${account.email} failed:`, e instanceof Error ? e.message : e);
    }
  }
}
