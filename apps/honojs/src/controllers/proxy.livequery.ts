...
export class ProxyController extends Hono {
  ...
  private authenticateProxy(req: Request): { ok: true } | { ok: false; response: Response } {
    const bearer = req.headers.get("authorization")?.match(/^Bearer\s+(.+)$/i)?.[1] ?? "";

    if (PROXY_API_KEY && bearer === PROXY_API_KEY) return { ok: true };

    if (!bearer) {
      return { ok: false, response: Response.json(
        { error: { message: `Unauthorized: provide API key "${PROXY_API_KEY}" or a PLUS account token.`, type: "auth" } },
        { status: 401 }
      )};
    }

    // Token khớp account đã có trong hệ thống
    const existing = this.accounts.getAccounts().find((a) => a.accessToken === bearer);
    if (existing) return { ok: true };

    // Thử decode JWT của OpenAI
    if (bearer.split(".").length === 3) {
      const payload = this.decodeJwtPayload(bearer);
      const authPayload = payload["https://api.openai.com/auth"] ?? {};
      const planType: string = (authPayload.chatgpt_plan_type ?? "").toLowerCase();
      const email: string = payload.email ?? payload["https://api.openai.com/profile"]?.email ?? "unknown";

      if (planType === "plus" || planType === "pro") {
        // Tự động thêm account PLUS vào danh sách
        const added = this.accounts.importFromTokens({ access_token: bearer });
        if (added) {
          console.log(`[proxy-auth] Auto-added ${planType.toUpperCase()} account: ${email}`);
          return { ok: true };
        }
      } else if (planType) {
        return { ok: false, response: Response.json(
          { error: { message: `Unauthorized: plan "${planType}" is not allowed, PLUS or PRO required.`, type: "auth" } },
          { status: 401 }
        )};
      }
    }
    ...
    return { ok: false, response: Response.json(
      { error: { message: "Unauthorized: invalid API key or account token.", type: "auth" } },
      { status: 401 }
    )};
  }
  ...
  private async handleProxyRoute(c: any): Promise<Response> {
    const req = c.req.raw;
    const auth = this.authenticateProxy(req);
