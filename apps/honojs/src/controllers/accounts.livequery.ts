...
export class AccountsController extends Hono {
  ...
  private runAction(action: string, ctx: LivequeryContext, origin: string): Promise<Response> | Response {
    switch (action) {
      ...
      case "reset-rate-limit": return this.resetRateLimit(ctx);
  ...
  private async resetRateLimit(ctx: LivequeryContext): Promise<Response> {
    const payload = (ctx.request.body ?? {}) as Record<string, unknown>;
    const id = String(ctx.livequery?.keys?.account_id ?? payload.id ?? "");
    if (!id) return error("BAD_REQUEST", "Missing account id");
    const result = await this.accounts.resetRateLimit(id);
    if (!result.ok) return error("BAD_REQUEST", result.error);
    this.logger.logEvent("rate_limit_reset", id);
    this.store.addReport({ type: "rate_limit_reset", accountId: id, timestamp: Date.now() });
    // Refresh quota để cập nhật rateLimitResetCount và trạng thái mới
    void this.accountService.fetchQuota();
    return json({ data: { ok: true, code: result.code } });
  }
