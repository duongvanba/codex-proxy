...
export function RemoteEnrollPanel({ accountId }: { accountId: string }) {
  ...
  async function handleDisconnect() {
    if (!confirm("Disconnect remote control?")) return;
    setLoading(true); setError(null);
    try {
    ...
    <div className="rc-panel">
      <div className="rc-enrollment">
        <div className="rc-enrollment-header">
          <span className="rc-label">Remote Control</span>
          <span className={`rc-status-badge ${status?.enrolled ? "enrolled" : "not-enrolled"}`}>
            {loading ? "..." : status?.enrolled ? "Connected" : "Not connected"}
          </span>
        </div>
        ...
        <div className="rc-enrollment-actions">
          {!status?.enrolled ? (
            <button disabled={loading} onClick={() => void handleEnroll()}>
              {loading ? <span className="inline-spinner compact" /> : null} Connect Remote
            </button>
          ) : (
            <button className="danger-btn" disabled={loading} onClick={() => void handleDisconnect()}>Disconnect</button>
          )}
          <button className="secondary-btn" disabled={loading} onClick={() => void loadStatus()}>Refresh</button>
        </div>
      </div>
      {status?.enrolled && activeHosts.length > 0 && (
        <div className="rc-hosts">
          <div className="rc-section-label">Remote machines</div>
      ...
      {status?.enrolled && (
        <div className="rc-shell">
          ...
          <div className="rc-shell-input-row">
            <input className="rc-shell-input" type="text" placeholder={canRunShell ? "Enter command..." : "No host selected"}
            ...
            <button disabled={!shellCmd.trim() || !canRunShell || shellRunning} onClick={() => void handleRunShell()}>
              {shellRunning ? <span className="inline-spinner compact" /> : "Run"}
            </button>
          </div>
          ...
          {(shellOutput || shellRunning) && (
            <div className="rc-shell-output-wrap">
              {lastExitCode !== null && (
                <div className={`rc-exit-code ${lastExitCode === 0 ? "ok" : "fail"}`}>Exit {lastExitCode}</div>
              )}
