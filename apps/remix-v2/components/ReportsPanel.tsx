  ...
  return (
    <Box as="section">
      <Flex align="center" justify="space-between" mb="2" h="8">
        <Heading size="xs" color="fg.muted" textTransform="uppercase" letterSpacing="wider" display="flex" alignItems="center" gap="2">
          Nhật ký {loading && <Spinner size="xs" />}
        </Heading>
      </Flex>
      <Box
        bg="bg.panel"
      ...
      >
        {reports.length === 0 && loading ? (
          <Flex align="center" gap="2" color="fg.muted" fontSize="sm" py="2">
            <Spinner size="xs" /> Đang tải nhật ký...
          </Flex>
        ) : reports.length === 0 ? (
          <Text color="fg.muted" fontSize="sm" py="2">Chưa có nhật ký.</Text>
        ) : (
