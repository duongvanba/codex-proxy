...
export async function fetchHosts(account: Account): Promise<HostItem[]> {
  const url = `${CHATGPT_BASE}/backend-api/codex/remote/control/environments?limit=100`;
  const res = await fetch(url, { headers: buildCodexHttpHeaders(account, "application/json") });
...
export async function fetchChats(
  account: Account,
  params: { taskFilter?: string; limit?: number; envId?: string; projectPath?: string } = {}
): Promise<ChatItem[]> {
  const qs = new URLSearchParams({
    task_filter: params.taskFilter ?? "current",
  ...
  if (params.envId) {
    tasks = tasks.filter((t) => t.environment_id === params.envId);
  }
  if (params.projectPath) {
    tasks = tasks.filter((t) => t.workspace_root === params.projectPath || t.workspaceRoot === params.projectPath);
  }
  return tasks.map((t) => ({
    ...t,
    ...
    environment_id: t.environment_id as string | undefined,
    created_at: t.created_at as string | undefined,
    updated_at: t.updated_at as string | undefined,
  }));
}

export async function fetchTurns(account: Account, chatId: string): Promise<{ turns: TurnItem[]; current_turn_id?: string }> {
...
async function whamPost(account: Account, path: string, body?: unknown): Promise<Record<string, unknown>> {
  const res = await fetch(`${CHATGPT_BASE}/backend-api${path}`, {
    method: "POST",
    headers: buildCodexHttpHeaders(account, "application/json"),
...
export async function createTask(
  ...
  const data = await whamPost(account, "/wham/tasks", body);
...
export async function sendFollowUp(
  ...
  const data = await whamPost(account, "/wham/tasks", { follow_up, input_items: params.input_items });
  return { ...data, task_id: String(data.task_id ?? data.id ?? taskId) };
}

export async function cancelTask(account: Account, taskId: string): Promise<void> {
  await whamPost(account, `/wham/tasks/${taskId}/cancel`);
}

export async function archiveTask(account: Account, taskId: string): Promise<void> {
  await whamPost(account, `/wham/tasks/${taskId}/archive`);
}

export async function recoverTask(account: Account, taskId: string): Promise<void> {
  await whamPost(account, `/wham/tasks/${taskId}/recover`);
}

export async function markTaskRead(account: Account, taskId: string): Promise<void> {
  await whamPost(account, `/wham/tasks/${taskId}/mark_read`);
}
