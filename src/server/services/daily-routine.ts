...
function codexHeadersForAccount(account: Account): Headers {
...
function warmUpBody(account: Account) {
...
async function warmUpAccount(account: Account): Promise<DailyRoutineAccountResult> {
  ...
  try {
    ...
    if (res.status === 429 || isLimitErrorText(text)) {
...
export async function runDailyRoutineForEveryAccount(onReport?: ReportFn): Promise<DailyRoutineResult> {
  const startedAt = Date.now();
  ...
  for (let i = 0; i < accounts.length; i += DEFAULT_CONCURRENCY) {
    ...
    const batchResults = await Promise.all(batch.map(warmUpAccount));
...
function zonedTimeToUtcMs(
  ...
  const actual = timeZoneParts(new Date(utcGuess), timeZone);
  const actualAsUtc = Date.UTC(actual.year, actual.month - 1, actual.day, actual.hour, actual.minute, actual.second);
  return utcGuess - (actualAsUtc - utcGuess);
}

export function nextDailyRoutineRunAt(
  now = new Date(),
  ...
  let target = zonedTimeToUtcMs(timeZone, parts.year, parts.month, parts.day, hour, minute);
  if (target <= now.getTime()) {
    const tomorrowUtc = new Date(Date.UTC(parts.year, parts.month - 1, parts.day + 1, hour, minute, 0));
    const tomorrow = timeZoneParts(tomorrowUtc, timeZone);
    target = zonedTimeToUtcMs(timeZone, tomorrow.year, tomorrow.month, tomorrow.day, hour, minute);
  }
  return new Date(target);
}

export function startDailyRoutineScheduler(onReport?: ReportFn) {
  ...
  const scheduleNext = () => {
    if (stopped) return;
    const next = nextDailyRoutineRunAt();
    ...
    timer = setTimeout(async () => {
      try {
        await runDailyRoutineForEveryAccount(onReport);
      } catch (error) {
        onReport?.({
